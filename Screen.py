...
class Screen:
    ...
    def get_scale_factor(cls) -> float:
        ...
        logging.info("Calculated macOS display scale factor: %sx", cls._scale_factor)
        return cls._scale_factor
    ...
    def adjust_coordinates(
        ...
        if scale_factor > 1:
            ...
            logging.debug(
                "Adjusted coordinates from (%s, %s) to (%s, %s)",
                x,
                y,
                adjusted_x,
                adjusted_y,
    ...
    @staticmethod
    def take_screenshot(
        output_filename: str = Constants.DEFAULT_SCREENSHOT_NAME,
    ) -> str:
        try:
